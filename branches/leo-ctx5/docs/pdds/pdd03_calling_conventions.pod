...
C<set_args> and C<set_returns>:
    ...
    0b1000000 OPT_COUNT
                A INT argument with this bit set receives the count
		of optional arguments found so far. The optional
		argument count is reset then, so that another argument
		with OPT_COUNT can be used to check for more optional
		arguments.
