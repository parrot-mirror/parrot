...
Make a multiple dispatch call. Similar in syntax to C<Parrot_pcc_invoke_method_from_c_args>, but
doesn't separate out the invocant before the signature since the call can have
multiple invocants. A significant part of the code from C<Parrot_pcc_invoke_method_from_c_args>
can be factored out to helper routines used by both C<Parrot_pcc_invoke_method_from_c_args> and
C<Parrot_mmd_invoke>.
