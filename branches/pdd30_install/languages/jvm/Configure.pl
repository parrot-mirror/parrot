...
# #############################################################################
# JVM class To PIR Translator, based on dotnet
...
# Parse command line.
my ( $parrot_path, $java_lib_path, $srm );
GetOptions(
    'parrot=s'  => \$parrot_path,
    'srm=s'     => \$srm,
    'java_home=s' => \$java_lib_path
);
...
sub generate_makefile {
    ...
    my $class_lib_make = generate_classlib_make($java_lib_path);

    # Generate Makefile in two steps
    my $conf = Parrot::Configure->new;
    $conf->data()->slurp();
    $conf->genfile('config/Makefile.in', 'config/Makefile.tmp');
    # and read temp. makefile template to fill in ${trans_java_lib_path}, ${trans_class_library}, srm
    open my $in_fh, '<', 'config/Makefile.tmp'
        or die "Unable to open config/Makefile.tmp\n";
    my $makefile = join( '', <$in_fh> );
    close $in_fh;

    my $local_java_lib_path = $java_lib_path || q{};
    $makefile =~ s/\$\{trans_java_lib_path\}/$local_java_lib_path/g;
...
sub generate_config_pm {
    ...
    # Read in template.
    open my $in_fh, '<', 'config/J2PConfig_pm.in'
        or die "Unable to open config/J2PConfig_pm.in\n";
    my $config_pm = join( '', <$in_fh> );
    close $in_fh;

    # Sub in config data.
    for ( keys %{$config} ) {
        # warnings flags aren't substituted in config/J2PConfig_pm.in; skip them
        next if m/^-W/g;
        $config_pm =~ s/\${$_}/$config->{$_}/g;
    }

    # Write.
    open my $out_fh, '>', 'config/J2PConfig.pm'
        or die "Unable to open config/J2PConfig.pm\n";
...
sub generate_classlib_make {
    ...
    if ( ! $java_class_lib) {
        ...
        return "\techo Oops, I was not configured with the path to Java class library.\n";
    }
    ...
    my $code = q{};
    foreach (@libs) {
        $code .= "\t\$(PARROT) -G class2pbc.pbc -f -p \"$java_class_lib/$_.dll\" > $_.pir\n";
