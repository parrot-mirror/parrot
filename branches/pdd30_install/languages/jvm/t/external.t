...
#
die unless compile_cs( "external.dll", <<'JAVA');
...
CSHARP
die unless compile_java( "t.class", <<'JAVA', '-r:external.dll' );
namespace Testing
{
    using System;
    using ExportsStuff;
...
map { unlink "$_" or warn "$! $_" } qw( external.dll external.pbc t.class t.pbc );
