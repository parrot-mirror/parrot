...
The file contains an entry for each bytecode instruction to translate. The entry
...
specified as one or more pairs of uppercase hexadecimal digits seperated by
spaces.
...
These specify the translation itself. Either the entry C<instruction> or C<pir> 
is required. That is, not both, but at least one.
...
Add some pir code before the pir or instruction code.
Multiple lines of C<pre_translate> are allowed.

This migth be used for enum_fixup for branch, arithmetic/logical and compare
ops.

  pre_translate = <<PIR
    (${STEMP0}, ${STACK0}, ${STACK1}) = fix_enum_operands(${STYPES}, ${STACK0}, 
                                                          ${STACK1})
    ${INS} = concat ${STEMP0}
  PIR
...
C<op> or C<load>. In the case of C<op>, it needs to populate C<${DTYPES}>. In the
case of C<load>, it needs to populate C<${LOADTYPE}>.
...
Constants as specified e.g. in .NET Partition II Section 22.1.15 will be set.

=head2 Meta-variables

=head3 ${STACK0}, ${STACK1}, ...

These refer to locations on the stack. C<${STACK0}> refers to the stack top,
C<${STACK1}> refers to the element second from the top, etc. Note that these
will be popped from the stack down to the lowest point in the stack tha is
accessed. or example, if C<${STACK0}> and C<${STACK2}> are used, then the second
location in the stack (which would be called C<${STACK1}>) will also be popped
off.

=head3 ${DEST0}, ${DEST1}, ...

For instructions in the C<op> class, these are the locations that the results
of the operation will be placed. For instructions in the C<load> class,
C<${DEST0}> is sometimes used to mean the register that the loaded content
will be placed in.  These are used when new data needs to be pushed onto the
stack. This works the opposite way round to the C<${STACKn}> meta-variables;
C<${DEST0}> will be pushed first, followed by C<${DEST1}>, etc. If this is
used when the class is anything other than C<op> or C<load>, or is used in a
C<load> and also mention C<${LOADREG}>, then a monkey may explode. Oh, and
you'll get an error.

=head3 ${ARG0}, ${ARG1}, ...

These refer to the arguments for the instruction, as specified in the
C<"arguments"> entry. Here, C<${ARG0}> is the first argument, C<${ARG1}> the
second, etc.

=head3 ${ITEMP0}, ${ITEMP1}, ...

These are temporary variables that can be used in any PIR code. They will
alway map to an C<I> parrot register. Do not assume anything about the
contents of these - they will likely contain junk from whatever used them
last.

=head3 ${NTEMP0}, ${NTEMP1}, ...

These are temporary variables that can be used in any PIR code. They will
alway map to a C<N> parrot register. Do not assume anything about the contents
of these - they will likely contain junk from whatever used them last.

=head3 ${STEMP0}, ${STEMP1}, ...

These are temporary variables that can be used in any PIR code. They will
alway map to a C<S> parrot register. Do not assume anything about the contents
of these - they will likely contain junk from whatever used them last.

=head3 ${PTEMP0}, ${PTEMP1}, ...

These are temporary variables that can be used in any PIR code. They will
alway map to a C<P> parrot register. Do not assume anything about the contents
of these - they will likely contain junk from whatever used them last.
...
This is the bytecode PMC, used for walking the bytecode. Hopefully, it
...
bytecode instructions and produces this PIR code, and that translator is written in
