...
The .NET CLI and Java JVM are stack based virtual machines whereas
Parrot is register based.  This document details what that means and
how the challenges the difference presents will be dealt with.
...
that need to be mapped to Parrot registers. The JVM architecture is the same.
...
before the method is called. They are accessed by a number of instructions
...
.NET CLI and JVM bytecode always retrieves parameters by number, the argument
number being a constant and thus known at translation time. Therefore, a
naming scheme that has the first parameter named "arg0", the second parameter
named "arg1" etc.  will be sufficient.
...
.NET CLI and he Java JVM bytecode retrieves parameters by number, the number
being a constant and thus known at translation time. Therefore, a naming
scheme that has the first local named "local0", the second local named
"local1" etc. will be sufficient.
