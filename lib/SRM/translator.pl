...
# Used by dotnet, WMLScript and jvm.

use strict;
use warnings;
use Getopt::Long;
use lib qw( lib ../../lib config );

# Grab parameters.
my ( $rules_file, $output_file, $srm_module, $config_package, @need_enum_fix, $HLLNAME );
GetOptions(
    "output=s" => \$output_file,
    "srm=s"    => \$srm_module,
    "config=s" => \$config_package,
) or usage();
$rules_file = shift @ARGV;
usage() if !$rules_file || @ARGV;

# Attempt to load stack to register mapping module.
require "SRM/$srm_module.pm";
my $srm = "SRM::$srm_module"->new();

require "$config_package.pm"; # defines the HLLNAME and magical fixups
{
    no strict 'refs';
    $HLLNAME = ${"${config_package}::HLLNAME"};
    @need_enum_fix = (defined @{"${config_package}::need_enum_fix"})
        ? @{"${config_package}::need_enum_fix"}
        : ();
}
...
sub parse_rules {
    ...
    while (<$fh>) {
        ...
        # If it's a blank line or a comment line, skip it.
...
TRANSPIR
    ...
    $pir .= <<'PIRCODE';
    # If we have exception handlers, need some initial stuff emitted.
    ehs = bc.get_eh()
...
PIR
    ...
    if ( @need_enum_fix and (grep { $rule->{'code'} eq $_ } @need_enum_fix )) {
        $rule->{'pre_translate'} = <<'PIR'
...
PIR
    ...
    if ( $rule->{'class'} eq 'op' ) {
        ...
        $pir .= sub_meta( $rule->{'pre_translate'}, $mv, "pre-translate for $rule->{'name'}" );
...
Usage:
  perl build/translator.pl src/translation.rules --srm Stack \
    --ouptput src/it.pir --config N2PConfig
USAGE
    exit(1);
}
