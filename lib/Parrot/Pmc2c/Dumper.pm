...
# $Id: Pmc2cMain.pm 18495 2007-05-11 00:50:42Z tewk $
...
sub dump_pmc {
    ...
    foreach my $pmc ( values %{$pmcs} ) {
        next if ( $pmc->name =~ /default\.dump$/ && $pmc->dump_is_current );
...
sub dump_is_newer {
    my $dumpfile = shift;
    my $pmcfile  = filename($dumpfile, '.pmc');
...
sub gen_parent_reverse_lookup_info {
    ...
    foreach my $vt_method_name ( @{ $vt->names } ) {
        ...
        unless ( $pmc->super_method($vt_method_name) ) {
            # XXX This is a quick hack to get the inheritance ParrotClass isa delegate
            # delegate has everything autogenerated, so these methods aren't seen and 
            # not inherited properly. The correct way would be to look at $class->implements
            # but when dumping there isn't a $class object
            my $super_method = ( $pmc->name eq 'ParrotObject' || $pmc->name eq 'ParrotClass' )
                ? 'delegate' : 'default';

            $pmc->super_method($vt_method_name, $super_method);
        }
    }
    return 1;
}
