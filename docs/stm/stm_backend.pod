...
An implementation may or may not support nested transactions. If it does
not, then starting a nested transaction will always fail.
...
=item int STM_start_transaction(Interp *interp)

Start a new (possibly nested) transaction. Returns true on success, false otherwise.
...
returns false, the transaction was aborted.

=item int STM_abort(Interp *)
...
Returns true if the current transaction is still valid (a commit would succeed), false otherwise.
