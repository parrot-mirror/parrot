...
STM implementation can be used from PIR and PASM. It also describes 
libraries that should be provided for language implementators to more
easily use the STM correctly (for example, while assuring that 
transactions are automatically aborted when their scope is abnormally exited).
...
other side effects -- this functionality, if provided, is the responsiblity
...
(Thus, an invalid transaction is guarenteed to fail to commit. A valid
...
If a inner transaction commits, its effects are still not seen until
...
STMRef, it does not attempt to make the transactions transparent. 
...
(guarenteed to be) protected from modification, but modifying it should be
